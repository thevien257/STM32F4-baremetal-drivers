...
void SPI2_IRQHandler(void) {
	SPI_TxRx_HandlingIT(&spi2_slave);
}
...
int main(void) {
	...
	while (1) {
		// Wait for master to deselect (NSS HIGH) - ready for next transaction
		while (!(GPIOB->IDR & (1 << 12)))
			;

		// Prepare for next transaction while NSS is HIGH
		txCompl = LOW;
		rxCompl = LOW;

		// This function will preload first byte and enable interrupts
		SPI_SendReceive_FullDuplex_IT(&spi2_slave, txData, rxData, 16);

		// Wait for master to select (NSS LOW)
		while (GPIOB->IDR & (1 << 12))
			;

		// Wait for transfer complete
		while (txCompl == LOW || rxCompl == LOW)
			;
